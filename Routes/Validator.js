...
Validator.Tags = {
   ...
   badValue: "badValue",            // Field has bad value.  P
   ...
   noOldPwd: "noOldPwd",            // Change of password requires OLD PW
   dupTitle: "dupTitle",            // Title duplicates an existing Conc title
...
Validator.prototype.check = function(test, tag, params, cb) {
   ...
   if (this.errors.length) {
      if (this.res) {
         ...
         else if (this.errors[this.errors.length - 1].tag
          === Validator.Tags.serverError) {
         ...
         this.res = null;   // Preclude repeated
      }
      if (cb)
         cb(this);
   }
